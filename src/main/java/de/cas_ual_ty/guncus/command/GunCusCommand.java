...
public class GunCusCommand
{
    public static void register(CommandDispatcher<CommandSource> dispatcher)
    {
        ...
        {
            return GunCusCommand.change(context.getSource(), true);
        }))).then(Commands.literal("off").executes((context) ->
        {
            return GunCusCommand.change(context.getSource(), false);
        })).then(Commands.literal("empty").executes((context) ->
        {
            return GunCusCommand.empty(context.getSource());
        })));
    }
    ...
    public static int change(CommandSource source, boolean on)
    {
        if(source.getEntity() instanceof PlayerEntity)
        {
            ...
            {
                if(player.getHeldItem(hand).getItem() instanceof GunItem)
                {
                    GunItem gun = (GunItem)player.getHeldItem(hand).getItem();
